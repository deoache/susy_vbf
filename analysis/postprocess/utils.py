...
"""
    ...
    stat_unc_f_total = f"{float(stat_unc_total):.2f}" if pd.notna(stat_unc_total) else 'nan'
    syst_unc_f_total = f"{(syst_unc_up_total + syst_unc_down_total) / 2:.2f}" if pd.notna(syst_unc_up_total) and pd.notna(syst_unc_down_total) else 'nan'
